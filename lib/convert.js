...
function buttonObj(nodeId, targetNodeId, normalSpriteUuid) {
    return { __type__: 'cc.Button', _name: '', _objFlags: 0, node: { __id__: nodeId }, _enabled: true,
        ...
        _N$normalSprite: normalSpriteUuid ? { __uuid__: normalSpriteUuid } : null, _N$pressedSprite: null, pressedSprite: null, _N$hoverSprite: null,
...
function labelOutlineObj(nodeId, color, width) {
    return { __type__: 'cc.LabelOutline', _name: '', _objFlags: 0, node: { __id__: nodeId }, _enabled: true,
        _color: { __type__: 'cc.Color', r: color.r, g: color.g, b: color.b, a: 255 }, _width: width, _id: '' };
}
function labelShadowObj(nodeId, color, offsetX, offsetY, blur) {
    return { __type__: 'cc.LabelShadow', _name: '', _objFlags: 0, node: { __id__: nodeId }, _enabled: true,
        _color: { __type__: 'cc.Color', r: color.r, g: color.g, b: color.b, a: 255 },
        _offset: { __type__: 'cc.Vec2', x: offsetX, y: offsetY }, _blur: blur, _id: '' };
}
...
function buildModel(psd, opts, report) {
    ...
    function walk(layer, depth) {
        ...
        let fontSize = null;
        let strokeColor = null, strokeFlag = false, outlineWidth = 0;
        let shadowColor = null, shadowOffsetX = 0, shadowOffsetY = 0, shadowBlur = 0;
        if (isText) {
            ...
            if (style.strokeFlag) {
                ...
                if (sc && typeof sc.r === 'number') {
                    strokeColor = { r: Math.round(sc.r), g: Math.round(sc.g), b: Math.round(sc.b) };
                } else {
            ...
            if (layer.effects) {
                ...
                if (!fx.disabled) {
                    if (fx.dropShadow && fx.dropShadow.length) {
                        for (var di = 0; di < fx.dropShadow.length; di++) {
                            var ds = fx.dropShadow[di];
                            if (ds.enabled) {
                                shadowColor = ds.color || { r: 0, g: 0, b: 0 };
                                var dist = ds.distance && ds.distance.value || 0;
                                var angleRad = (ds.angle || 0) * Math.PI / 180;
                                shadowOffsetX = Math.round(dist * Math.sin(angleRad));
                                shadowOffsetY = Math.round(-dist * Math.cos(angleRad));
                                shadowBlur = ds.size && ds.size.value || 0;
                                break;
                            }
                        }
                    }
                    if (!strokeFlag && fx.stroke && fx.stroke.length) {
                        for (var si = 0; si < fx.stroke.length; si++) {
                            var st = fx.stroke[si];
                            if (st.enabled) {
                                strokeFlag = true;
                                outlineWidth = st.size && st.size.value || 1;
                                strokeColor = st.color || { r: 0, g: 0, b: 0 };
                                break;
                            }
                        }
                    }
        ...
        return { kind: kind, info, crop: crop,
            ...
            strokeFlag: strokeFlag, strokeColor: strokeColor, outlineWidth: outlineWidth,
            shadowColor: shadowColor, shadowOffsetX: shadowOffsetX, shadowOffsetY: shadowOffsetY, shadowBlur: shadowBlur,
            px: { left, top, right, bottom }, width: w, height: h,
            opacity: Math.round((layer.opacity == null ? 1 : layer.opacity) * 255),
            color: { r: 255, g: 255, b: 255 }, active: !hidden, layer, children: [] };
    }
    const roots = [];
...
async function convert(opts) {
    ...
    function emit(node, parentId, parentAbs) {
        ...
        if (node.kind === 'sprite' || node.kind === 'sprite-sliced') {
            ...
            if (node.strokeFlag && node.strokeColor) {
                const olCid = out.length; out.push(labelOutlineObj(nodeId, node.strokeColor, node.outlineWidth || 1)); compIds.push(olCid);
            }
            if (node.shadowColor) {
                const shCid = out.length; out.push(labelShadowObj(nodeId, node.shadowColor, node.shadowOffsetX, node.shadowOffsetY, node.shadowBlur)); compIds.push(shCid);
            }
        } else if (node.kind === 'button') {
            var bgSpriteUuid = null, bgSliced = false, bgChildIndex = -1;
            var children = node.children || [];
            if (node.layer && node.layer.imageData) {
                var selfImg = node.info.ref ? resolveByRef(node, table) : resolveImage(node);
                bgSpriteUuid = selfImg ? selfImg.spriteUuid : null;
            } else {
                for (var bi = 0; bi < children.length; bi++) {
                    if (children[bi].kind === 'sprite' || children[bi].kind === 'sprite-sliced') {
                        bgChildIndex = bi;
                        bgSliced = children[bi].kind === 'sprite-sliced';
                        var bgImg = children[bi].info.ref ? resolveByRef(children[bi], table) : resolveImage(children[bi]);
                        bgSpriteUuid = bgImg ? bgImg.spriteUuid : null;
                        break;
                    }
                }
                if (bgChildIndex >= 0) children[bgChildIndex]._skipEmit = true;
            }
            var spriteCid = out.length; out.push(spriteObj(nodeId, bgSpriteUuid, bgSliced)); compIds.push(spriteCid);
            var btnCid = out.length; out.push(buttonObj(nodeId, nodeId, bgSpriteUuid)); compIds.push(btnCid);
        }
        else if (node.kind === 'layout') { const cid = out.length; out.push(layoutObj(nodeId, node.info.mods.layout || 'h')); compIds.push(cid); }
        for (const child of (node.children || [])) { if (child._skipEmit) continue; const id = emit(child, nodeId, abs); childIds.push(id); }
